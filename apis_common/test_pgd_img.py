...
def single_gpu_test(model, data_loader,
    ...
    scattered_result_dir = scattered_result_prefix +'_eps'+eps255+'_step'+max_step
    eps255 = float(eps255)
    ...
    for data_i, data_out in enumerate(data_loader):
        ...
        # pgd 是随机初始化的 random start
        delta = torch.rand_like(orig_imgs_input)*2*eps01 - eps01
            ...
            for step in range(max_step):
                ...
                try:
                    print('attack step:', step, 
                        ...
                        'pgd_eps:', round(float(eps01),5),
                        'pgd_lr:', round(float(lr),5),
                        )
